...
Da $\msegment{MF}=r_I(W)$ gerade der Radius der Inkugel des Würfels und
$\msegment{MP}=r_U(O)$ der Radius der Umkugel des Oktaeders ist, ergibt sich
\begin{gather*}
  r_U(W)\m r_I(O)= r_I(W)\m r_U(O)=r_k^2,
\end{gather*}
wobei $r_K$ der Radius der gemeinsamen Kantenkugel ist. Es folgt
\begin{gather*}
  \frac{r_I(W)}{r_U(W)}=\frac{r_I(O)}{r_U(O)},
\end{gather*}
wobei diese Beziehung invariant bzgl. der konkreten Größe der Kantenkugel und
damit allgemeingültig ist.

Dieselben Überlegungen zeigen, dass auch für Dodekaeder und Ikosaeder in
dualer Lage
\begin{gather*}
  r_U(D)\m r_I(I)= r_I(D)\m r_U(I)=r_k^2
  \intertext{und allgemein}
  \frac{r_I(D)}{r_U(D)}=\frac{r_I(I)}{r_U(I)}
\end{gather*}
gilt.   Das Tetraeder ist zu sich selbst dual und erfüllt damit die Beziehung 
\begin{gather*}
  r_U(T)\m r_I(T)= r_k(T)^2. 
\end{gather*}
\emph{Anmerkung:} Während das Tetraeder und seine Entsprechung in dualer Lage
auch eine gemeinsame Inkugel und eine gemeinsame Umkugel haben, fallen die
Umkugeln bzw.  die Inkugeln für Würfel und Oktaeder in dualer Lage bzw.
Dodekaeder und Ikosaeder in dualer Lage nicht zusammen.  Davon kann man sich
etwa an Hand der genauen Formeln für die verschiedenen Radien überzeugen, die
wir im Weiteren berechnen werden.
...
\begin{center}
  ...
  \emph{Abbildung 3:} Platonische Körper in dualer Lage. Quelle: Wikipedia,
  T. Piesk, CC-BY 4.0
\end{center}
...
graebe (2023-10-29): Überarbeitung für einen Vortrag in der LSGM-Herbstschule.
Konstruktion der Dualität ergänzt.
\end{attribution}
