...
\author{Helmut König, Chemnitz} 
\date{4. Februar 2005}

\begin{document}
\maketitle

Wenn man im außerunterrichtlichen Bereich bei den Schülern die Fähigkeit zum
problemlösenden Denken entwickeln will, dann sollte man nicht vergessen,
bisweilen auch nach Fehlern in vorgelegten Lösungen suchen zu lassen. Ein
hierfür gut geeignetes Beispiel soll hier vorgestellt werden.
...
\subsection*{Die Aufgabenstellung}
  ...
  $\ksegment{BD}$ im Punkt $P$ und $Q$ sei der Fußpunkt des Lots von $P$ auf
  $\ksegment{BC}$. Die Strecke $\ksegment{MD}$ schneide die Diagonale
  $\ksegment{AC}$ im Punkt $S$ und $R$ sei der Fußpunkt des Lots von $S$ auf
  $\ksegment{BC}$.
  ...
  und dessen Flächeninhalt gleich dem neunten Teil des Flächeninhalts von
  $ABCD$ ist!


\subsection*{Auftrag an Schüler}

Beweise den ersten Teil der Behauptung! 

Prüfe nach, ob die folgenden drei Beweise des zweiten Teils der Behauptung
korrekt sind!  Wenn dies nicht der Fall ist, dann gib an, wo der Fehler
steckt!

\begin{minipage}{0.75\textwidth}
  \ul{1. Beweis}: Man teile die Seiten von $ABCD$ jeweils in drei gleiche
  ...
  Seitenlängen sich wie $1:2$ verhalten. Das rechts neben dem zentralen
...
\begin{minipage}{0.7\textwidth}
  \ul{2. Beweis}: Die Verlängerung von $\ksegment{QP}$ über $P$ hinaus
  schneide $\ksegment{AC}$ in $T$ und $\ksegment{AD}$ in $Q'$; die
  Verlängerung von $\ksegment{RS}$ über $S$ hinaus schneide $\ksegment{BD}$ in
  ...
  ist und dass die Gerade $\kline{QQ'}$ in sich übergeht. Folglich geht $P$ in
  $T$ und analog auch $S$ in $U$ über.
...
\bigskip
  ...
  Führt man anschließend noch die Spiegelung an der Symmetrieachse $n$ durch,
  dann erkennt man, dass dies für alle neun Teilrechtecke zutrifft. 

  Hieraus folgt unmittelbar, dass alle neun Teilrechtecke und daher auch unser
  Rechteck $PQRS$ einen Flächeninhalt besitzen, der gleich dem neunten Teil
  des Flächeninhalts von $ABCD$ ist.
\bigskip

\begin{minipage}{0.7\textwidth}
  \ul{3. Beweis}: Seien $T$ und $U$ die im 2.~Beweis eingeführten Hilfspunkte.
...
\subsection*{Auswertung mit den Schülern}
...
Man könnte auch sagen, dass hier nicht unser Satz sondern eine Umkehrung
...
{\glqq}zentrale Rechteck{\grqq} nicht kongruent sein müssen erkennt man, wenn
man das {\glqq}zentrale Rechteck{\grqq} ähnlich vergrößert oder verkleinert.

In diesem Fall liegt eine {\em Beweislücke} vor. 

{\bf Der 3. Beweis ist unkorrekt.}\\
Die für den Beweis unverzichtbare {\em Feststellung}
$\kline{AC}\parallel\kline{PR}$ wurde {\em der Anschauung entnommen}, aber
{\em nicht aus den Voraussetzungen hergeleitet}. (Dies wäre mit dem
Hilfsmittel Ähnlichkeit/Strahlensätze möglich.)
